...
test.describe('Sistema de Colaborador - Lançamento de Produção', () => {
  ...
  test('deve lançar produção com sucesso', async ({ page }) => {
    ...
    const successToast = page.locator('.Toastify__toast--success, .toast-success, [role="alert"]:has-text("sucesso")');
  ...
  test('deve mostrar erro ao tentar duplicata', async ({ page }) => {
    ...
    const errorToast = page.locator('.Toastify__toast--error, .toast-error, [role="alert"]:has-text("duplicada")');
  ...
  test('deve mostrar erro ao pular etapa', async ({ page }) => {
    ...
    // Verificar toast de erro
    const errorToast = page.locator('.Toastify__toast--error, .toast-error, [role="alert"]:has-text("Sequência")');
    await expect(errorToast.first()).toBeVisible({ timeout: 5000 });
  });
