...
export function createOperacionalRecebimentoRoutes(): FastifyPluginAsync {
  return async (server: FastifyInstance): Promise<void> => {
    ...
    server.get(
      '/operacional/setores-recebimento',
      {
        schema: {
          tags: ['operacional'],
          summary: 'Listar setores de recebimento ativos',
          security: [{ bearerAuth: [] }],
        },
    ...
    server.post(
      '/operacional/setores-recebimento',
      {
        schema: {
          tags: ['operacional'],
          summary: 'Criar setor de recebimento',
          security: [{ bearerAuth: [] }],
    ...
    server.get(
      '/operacional/classificacoes-recebimento',
      {
        schema: {
          tags: ['operacional'],
          summary: 'Listar classificações de recebimento ativas',
          security: [{ bearerAuth: [] }],
        },
        preHandler: [server.authenticate, authorize('operador', 'administrador')],
      },
    ...
    server.post(
      '/operacional/classificacoes-recebimento',
      {
        schema: {
          tags: ['operacional'],
          summary: 'Criar classificação de recebimento',
          security: [{ bearerAuth: [] }],
          body: { type: 'object', required: ['nome'], properties: { nome: { type: 'string' } } },
          response: {
            201: { type: 'object', additionalProperties: true },
    ...
    server.get(
      '/operacional/repositorios/:id/recebimento-processos',
      {
        schema: {
          tags: ['operacional'],
          summary: 'Listar processos de recebimento de um repositório',
          security: [{ bearerAuth: [] }],
      ...
      async (request, reply) => {
        try {
          ...
          const message =
            error instanceof Error ? error.message : 'Erro ao listar processos do recebimento';
          return sendDatabaseError(reply, error, message);
        }
      }
    );
    ...
    server.post(
      '/operacional/repositorios/:id/recebimento-processos',
      {
        schema: {
          tags: ['operacional'],
          summary: 'Criar processo de recebimento em repositório',
          security: [{ bearerAuth: [] }],
          params: { type: 'object', properties: { id: { type: 'string' } }, required: ['id'] },
          response: {
            201: { type: 'object', additionalProperties: true },
        ...
        preHandler: [
          server.authenticate,
          authorize('operador', 'administrador'),
          validateBody(criarProcessoRecebimentoSchema),
        ],
      },
      async (request, reply) => {
        try {
          ...
          const message =
            error instanceof Error ? error.message : 'Erro ao salvar processo do recebimento';
          return reply.status(400).send({ error: message });
        }
      }
    );
    ...
    server.delete(
      '/operacional/recebimento-processos/:processoId',
      {
        schema: {
          tags: ['operacional'],
          summary: 'Excluir processo de recebimento',
          security: [{ bearerAuth: [] }],
          params: {
            type: 'object',
            properties: { processoId: { type: 'string' } },
            required: ['processoId'],
          },
        },
        preHandler: [server.authenticate, authorize('operador', 'administrador')],
      ...
      async (request, reply) => {
        try {
          ...
          await server.database.query('DELETE FROM recebimento_processos WHERE id = $1', [
            processoId,
          ]);
    ...
    server.post(
      '/operacional/recebimento-processos/:processoId/volumes',
      {
        schema: {
          tags: ['operacional'],
          summary: 'Adicionar volume a processo',
          security: [{ bearerAuth: [] }],
          params: {
            type: 'object',
            properties: { processoId: { type: 'string' } },
        ...
        preHandler: [
          server.authenticate,
          authorize('operador', 'administrador'),
          validateBody(criarVolumeSchema),
        ],
      },
      async (request, reply) => {
        try {
    ...
    server.post(
      '/operacional/recebimento-processos/:processoId/apensos',
      {
        schema: {
          tags: ['operacional'],
          summary: 'Adicionar apenso a processo',
          security: [{ bearerAuth: [] }],
          params: {
            type: 'object',
            properties: { processoId: { type: 'string' } },
        ...
        preHandler: [
          server.authenticate,
          authorize('operador', 'administrador'),
          validateBody(criarApensoSchema),
        ],
      },
      async (request, reply) => {
        try {
    ...
    server.delete(
      '/operacional/recebimento-apensos/:apensoId',
      {
        schema: {
          tags: ['operacional'],
          summary: 'Excluir apenso',
          security: [{ bearerAuth: [] }],
          params: {
            type: 'object',
            properties: { apensoId: { type: 'string' } },
            required: ['apensoId'],
          },
        },
        preHandler: [server.authenticate, authorize('operador', 'administrador')],
    ...
    server.get(
      '/operacional/repositorios/:id/documentos-recebimento',
      {
        schema: {
          tags: ['operacional'],
          summary: 'Listar documentos de recebimento (formato legado)',
          security: [{ bearerAuth: [] }],
          params: { type: 'object', properties: { id: { type: 'string' } }, required: ['id'] },
        },
        preHandler: [server.authenticate, authorize('operador', 'administrador')],
      ...
      async (request, reply) => {
        try {
          ...
          const message =
            error instanceof Error ? error.message : 'Erro ao listar documentos de recebimento';
          return sendDatabaseError(reply, error, message);
        }
      }
    );
