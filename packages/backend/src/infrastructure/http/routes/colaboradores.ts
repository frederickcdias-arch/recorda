...
export function createColaboradoresRoutes(): FastifyPluginAsync {
  return async (server: FastifyInstance): Promise<void> => {
    ...
    server.get('/coordenadorias', {
      schema: {
        tags: ['colaboradores'],
        summary: 'Listar coordenadorias ativas',
        security: [{ bearerAuth: [] }],
        response: {
          200: { type: 'array', items: { type: 'object', properties: { id: { type: 'string' }, nome: { type: 'string' }, sigla: { type: 'string' } } } },
    ...
    server.get('/colaboradores', {
      ...
      try {
        const { nome, coordenadoriaId, ativo, limite = 50, pagina = 1 } = request.query as {
          nome?: string;
          coordenadoriaId?: string;
          ativo?: string;
          limite?: number;
    ...
    server.post<{ Body: CriarColaboradorBody }>('/colaboradores', {
      ...
      try {
        ...
        if (!nome || !matricula || !coordenadoriaId) {
          return reply.status(400).send({ error: 'Nome, matrícula e coordenadoria são obrigatórios' });
        }
    ...
    server.put<{ Params: { id: string }; Body: CriarColaboradorBody }>('/colaboradores/:id', {
      preHandler: [server.authenticate, authorize('administrador')],
    }, async (request, reply) => {
      try {
        ...
        if (matriculaResult.rows.length > 0) {
          return reply.status(409).send({ error: 'Matrícula já cadastrada para outro colaborador' });
        }
    ...
    server.patch<{ Params: { id: string } }>('/colaboradores/:id/toggle-ativo', {
      preHandler: [server.authenticate, authorize('administrador')],
    }, async (request, reply) => {
      try {
        const { id } = request.params;
    ...
    server.get<{ Params: { id: string } }>('/colaboradores/:id', {
      preHandler: [server.authenticate],
    }, async (request, reply) => {
      try {
        const { id } = request.params;
