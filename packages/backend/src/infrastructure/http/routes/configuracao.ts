...
export function createConfiguracaoRoutes(): FastifyPluginAsync {
  return async (server: FastifyInstance): Promise<void> => {
    ...
    server.put<{ Body: ConfiguracaoEmpresaInput }>(
      '/configuracao/empresa',
      {
        schema: {
          tags: ['configuracao'],
      ...
      async (request, reply) => {
        try {
          ...
          if (existsResult.rows.length === 0) {
            ...
            return reply
              .status(201)
              .send(mapRowToConfig(insertResult.rows[0] as Record<string, unknown>));
          }
          ...
          return reply
            .status(200)
            .send(mapRowToConfig(updateResult.rows[0] as Record<string, unknown>));
        } catch (error) {
      ...
      async (request, reply) => {
        try {
          ...
          if (!allowedTypes.includes(data.mimetype)) {
            return reply
              .status(400)
              .send({ error: 'Tipo de arquivo não permitido. Use PNG, JPG, SVG ou WebP.' });
          }
    ...
    server.get(
      '/configuracao/empresa/logo/arquivo',
      {
        schema: {
          tags: ['configuracao'],
          summary: 'Servir arquivo da logo',
    ...
    server.get(
      ...
      async (_request, reply) => {
        ...
        `);
          ...
          return reply.status(200).send({
            projetos: Array.from(uniqueMap.values()).map((projeto) => ({
    ...
    server.post<{ Body: ConfiguracaoProjetoInput }>(
      '/configuracao/projetos',
      {
        schema: {
          tags: ['configuracao'],
    ...
    server.put<{ Params: { id: string }; Body: ConfiguracaoProjetoInput }>(
      '/configuracao/projetos/:id',
      {
        schema: {
          tags: ['configuracao'],
    ...
    server.patch<{ Params: { id: string } }>(
      '/configuracao/projetos/:id/toggle-ativo',
      {
        schema: {
          tags: ['configuracao'],
      ...
      async (request, reply) => {
        try {
          ...
          const message =
            error instanceof Error ? error.message : 'Erro ao alterar status do projeto';
          return reply.status(500).send({ error: message });
        }
      }
    );
