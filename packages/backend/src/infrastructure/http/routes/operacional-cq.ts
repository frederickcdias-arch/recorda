...
export function createOperacionalCQRoutes(): FastifyPluginAsync {
  return async (server: FastifyInstance): Promise<void> => {
    ...
    server.post(
      '/operacional/lotes-cq',
      {
        ...
        preHandler: [
          ...
          validateBody(criarLoteCQSchema),
        ],
      },
      async (request, reply) => {
        try {
    ...
    server.patch(
      '/operacional/lotes-cq/:id/itens/:itemId',
      {
        preHandler: [
          server.authenticate,
          authorize('administrador'),
          validateBody(auditarItemCQSchema),
        ],
      },
      async (request, reply) => {
        try {
          ...
          const message =
            error instanceof Error ? error.message : 'Erro ao registrar resultado de CQ';
          return reply.status(400).send({ error: message });
        }
      }
    );
    ...
    server.post(
      '/operacional/lotes-cq/:id/fechar',
      {
    ...
    server.post(
      '/operacional/lotes-cq/:id/relatorio-entrega',
      {
        preHandler: [server.authenticate, authorize('administrador')],
      },
      async (request, reply) => {
        try {
          ...
          const message =
            error instanceof Error ? error.message : 'Erro ao gerar relatório de entrega';
          return reply.status(400).send({ error: message });
        }
      }
    );
    ...
    server.get(
      '/operacional/repositorios/:id/cq-avaliacoes',
      {
        schema: {
          tags: ['operacional-cq'],
    ...
    server.put(
      '/operacional/repositorios/:id/cq-avaliacoes/:processoId',
      {
        schema: {
          tags: ['operacional-cq'],
    ...
    server.post(
      '/operacional/repositorios/:id/cq-aprovar-todos',
      {
        schema: {
          tags: ['operacional-cq'],
      ...
      async (request, reply) => {
        try {
          ...
          if (parseInt(docsCheck.rows[0]?.total ?? '0', 10) === 0) {
            return reply
              .status(400)
              .send({ error: 'Repositório não possui processos/apensos para aprovação.' });
          }
          ...
          if (
            ...
            return reply
              .status(500)
              .send({ error: `${schemaDesatualizadoMsg} Detalhe: ${message}` });
          }
    ...
    server.post(
      '/operacional/repositorios/:id/cq-concluir',
      {
        schema: {
          tags: ['operacional-cq'],
      ...
      async (request, reply) => {
        try {
          ...
          if (total === 0) {
            return reply
              .status(400)
              .send({ error: 'Repositório não possui processos cadastrados.' });
          }
          if (pendentes > 0) {
            return reply.status(400).send({
    ...
    server.post(
      '/operacional/repositorios/:id/cq-devolver',
      {
        schema: {
          tags: ['operacional-cq'],
    ...
    server.post(
      '/operacional/repositorios/:id/termo-correcao',
      {
        schema: {
          tags: ['operacional-cq'],
      ...
      async (request, reply) => {
        try {
          ...
          const message =
            error instanceof Error ? error.message : 'Erro ao gerar termo de correção';
          return reply.status(400).send({ error: message });
        }
      }
    );
    ...
    server.post(
      '/operacional/repositorios/:id/termo-devolucao',
      {
        schema: {
          tags: ['operacional-cq'],
      ...
      async (request, reply) => {
        try {
          ...
          const message =
            error instanceof Error ? error.message : 'Erro ao gerar termo de devolução';
          return reply.status(400).send({ error: message });
        }
      }
    );
    ...
    server.get(
      '/operacional/relatorios/:id/download',
      {
        preHandler: [
          async (request) => {
      ...
      async (request, reply) => {
        try {
          ...
          if (format === 'csv') {
            ...
            const snapshot = snapshotResult.rows[0]?.dados_snapshot as
              | Record<string, unknown>
              | undefined;
            const processos = Array.isArray(snapshot?.processos)
              ? (snapshot.processos as Array<Record<string, unknown>>)
              : [];
            ...
            return reply
              ...
              .header(
                'Content-Disposition',
                `attachment; filename="relatorio-recebimento-${id}.csv"`
              )
              .send(csvContent);
          }
