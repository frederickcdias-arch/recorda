  ...
  return async (server: FastifyInstance): Promise<void> => {
    ...
    const gerarRelatorioRecebimento = async (repositorioIds: string[], userId: string) => {
      ...
      for (const row of processosResult.rows) {
        ...
        for (const ap of apensos) {
          processos.push({
            ...
            volume:
              ap.volume_total > 0
                ? `${ap.volume_atual} de ${ap.volume_total}`
                : String(ap.volume_atual),
            numeroCaixas: 0,
            caixaNova: false,
            isApenso: true,
      ...
      const empresaResult = await server.database.query(
        `SELECT nome, logo_url, logo_data, exibir_logo_relatorio, logo_largura_relatorio, logo_alinhamento_relatorio, logo_deslocamento_y_relatorio
    ...
    const gerarRelatorioProducao = async (repositorioId: string, userId: string) => {
      ...
      const totalQuantidade = registrosResult.rows.reduce(
        (total, row) => total + Number(row.quantidade ?? 0),
        0
      );
      const snapshot = {
        repositorio,
    ...
    server.get(
      '/operacional/checklist-modelos',
      {
        schema: {
          tags: ['operacional'],
          summary: 'Listar modelos de checklist',
          security: [{ bearerAuth: [] }],
          querystring: {
            type: 'object',
      ...
      async (request, reply) => {
        try {
          ...
          const message =
            error instanceof Error ? error.message : 'Erro ao listar modelos de checklist';
    ...
    server.post(
      '/operacional/checklist-modelos',
      {
        schema: {
          tags: ['operacional'],
          summary: 'Criar modelo de checklist',
          security: [{ bearerAuth: [] }],
        },
        preHandler: [
          server.authenticate,
          authorize('administrador'),
          validateBody(criarModeloChecklistSchema),
        ],
      },
      async (request, reply) => {
        try {
          ...
          if (!body.codigo || !body.descricao || !body.etapa) {
            return reply
              .status(400)
              .send({ error: 'Campos obrigatórios: codigo, descricao, etapa' });
          }
          ...
          const message =
            error instanceof Error ? error.message : 'Erro ao criar modelo de checklist';
          if (
            message.includes('checklist_modelos_codigo_key') ||
    ...
    server.put(
      '/operacional/checklist-modelos/:id',
      {
        schema: {
          tags: ['operacional'],
          summary: 'Atualizar modelo de checklist',
          security: [{ bearerAuth: [] }],
        ...
        preHandler: [
          server.authenticate,
          authorize('administrador'),
          validateBody(atualizarModeloChecklistSchema),
        ],
      },
      async (request, reply) => {
        try {
          ...
          const message =
            error instanceof Error ? error.message : 'Erro ao atualizar modelo de checklist';
          return reply.status(400).send({ error: message });
        }
      }
    );
    ...
    server.patch(
      '/operacional/checklist-modelos/:id/toggle-ativo',
      {
        schema: {
          tags: ['operacional'],
          summary: 'Ativar/desativar modelo de checklist',
          security: [{ bearerAuth: [] }],
          params: { type: 'object', properties: { id: { type: 'string' } }, required: ['id'] },
        },
        preHandler: [server.authenticate, authorize('administrador')],
      ...
      async (request, reply) => {
        try {
          ...
          const message =
            error instanceof Error ? error.message : 'Erro ao alterar status do modelo';
          return reply.status(400).send({ error: message });
        }
      }
    );
    ...
    server.post(
      '/operacional/repositorios/:id/checklists',
      {
        schema: {
          tags: ['operacional'],
          summary: 'Abrir checklist de etapa para repositório',
          security: [{ bearerAuth: [] }],
          params: { type: 'object', properties: { id: { type: 'string' } }, required: ['id'] },
        },
        preHandler: [
          ...
          validateBody(abrirChecklistSchema),
        ],
      },
      async (request, reply) => {
        try {
          ...
          if (message.includes('uk_checklists_ativos_por_etapa')) {
            return reply
              .status(409)
              .send({ error: 'Já existe checklist ativo para esta etapa e repositório' });
          }
          return sendDatabaseError(reply, error, message);
        }
      }
    ...
    server.get(
      '/operacional/repositorios/:id/checklists',
      {
        schema: {
          tags: ['operacional'],
          summary: 'Listar checklists de repositório',
          security: [{ bearerAuth: [] }],
          params: { type: 'object', properties: { id: { type: 'string' } }, required: ['id'] },
        },
    ...
    server.get(
      '/operacional/checklists/:id',
      {
        schema: {
          tags: ['operacional'],
          summary: 'Buscar checklist por ID com itens',
          security: [{ bearerAuth: [] }],
          params: { type: 'object', properties: { id: { type: 'string' } }, required: ['id'] },
        },
        preHandler: [server.authenticate, authorize('operador', 'administrador')],
    ...
    server.post(
      '/operacional/checklists/:id/itens',
      {
        schema: {
          tags: ['operacional'],
          summary: 'Registrar item de checklist',
          security: [{ bearerAuth: [] }],
          params: { type: 'object', properties: { id: { type: 'string' } }, required: ['id'] },
        },
        preHandler: [
          server.authenticate,
          authorize('operador', 'administrador'),
          validateBody(registrarItemChecklistSchema),
        ],
      },
      async (request, reply) => {
        try {
          ...
          const message =
            error instanceof Error ? error.message : 'Erro ao registrar item do checklist';
          return sendDatabaseError(reply, error, message);
        }
      }
    );
    ...
    server.post(
      '/operacional/checklists/:id/concluir',
      {
        schema: {
          tags: ['operacional'],
          summary: 'Concluir checklist (salva itens + conclui em uma chamada)',
          security: [{ bearerAuth: [] }],
          params: { type: 'object', properties: { id: { type: 'string' } }, required: ['id'] },
        },
        preHandler: [
          server.authenticate,
          authorize('operador', 'administrador'),
          validateBody(concluirChecklistSchema),
        ],
      },
      async (request, reply) => {
        try {
    ...
    server.post(
      '/operacional/repositorios/:id/producao',
      {
        schema: {
          tags: ['operacional'],
          summary: 'Registrar produção em repositório',
          security: [{ bearerAuth: [] }],
          params: { type: 'object', properties: { id: { type: 'string' } }, required: ['id'] },
        },
        preHandler: [
          ...
          const result = await server.database.query(
           ...
           RETURNING *`,
            [
              ...
              body.checklistId,
              user.id,
              Math.max(Number(body.quantidade ?? 1), 1),
              JSON.stringify(body.marcadores ?? {}),
            ]
          );
    ...
    server.post(
      '/operacional/relatorio-recebimento',
      {
        schema: {
          tags: ['operacional'],
          summary: 'Gerar relatório de recebimento (múltiplos repositórios)',
          security: [{ bearerAuth: [] }],
        },
        preHandler: [
          server.authenticate,
          authorize('operador', 'administrador'),
          validateBody(relatorioRecebimentoSchema),
        ],
      },
      async (request, reply) => {
        try {
    ...
    server.post(
      '/operacional/repositorios/:id/relatorio-recebimento',
      {
        schema: {
          tags: ['operacional'],
          summary: 'Gerar relatório de recebimento (repositório único, compat)',
          security: [{ bearerAuth: [] }],
          params: { type: 'object', properties: { id: { type: 'string' } }, required: ['id'] },
        },
        preHandler: [server.authenticate, authorize('operador', 'administrador')],
    ...
    server.post(
      '/operacional/repositorios/:id/relatorio-producao',
      {
        schema: {
          tags: ['operacional'],
    ...
    server.get(
      '/operacional/repositorios/:id/relatorios',
      {
        schema: {
          tags: ['operacional'],
          summary: 'Listar relatórios gerados de repositório',
          security: [{ bearerAuth: [] }],
          params: { type: 'object', properties: { id: { type: 'string' } }, required: ['id'] },
        },
        preHandler: [server.authenticate, authorize('operador', 'administrador')],
    ...
    server.post(
      '/operacional/repositorios/:id/excecoes',
      {
        schema: {
          tags: ['operacional'],
          summary: 'Registrar exceção em repositório',
          security: [{ bearerAuth: [] }],
          params: { type: 'object', properties: { id: { type: 'string' } }, required: ['id'] },
        },
        preHandler: [server.authenticate, authorize('operador', 'administrador')],
    ...
    server.patch(
      '/operacional/repositorios/:id/entregar',
      {
        schema: {
          tags: ['operacional'],
          summary: 'Marcar repositório como entregue',
          security: [{ bearerAuth: [] }],
          params: { type: 'object', properties: { id: { type: 'string' } }, required: ['id'] },
        },
        preHandler: [server.authenticate, authorize('operador', 'administrador')],
    ...
    server.patch(
      '/operacional/repositorios/:id/avancar',
      {
        schema: {
          tags: ['operacional'],
          summary: 'Avançar repositório para próxima etapa',
          security: [{ bearerAuth: [] }],
          params: { type: 'object', properties: { id: { type: 'string' } }, required: ['id'] },
        },
        preHandler: [server.authenticate, authorize('operador', 'administrador')],
        ...
        try {
          ...
          if (repositorio.etapa_atual === 'DIGITALIZACAO') {
            ...
            if (!seadeskCheck.rows[0]?.confirmado) {
              ...
              return reply.status(400).send({
                error:
                  'Confirmação de envio Seadesk é obrigatória antes de avançar da Digitalização',
