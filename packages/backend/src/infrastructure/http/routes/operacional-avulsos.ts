  ...
  return async (server: FastifyInstance): Promise<void> => {
    ...
    server.post(
      '/operacional/recebimento-avulsos/ocr-preview',
      {
        schema: {
          tags: ['operacional'],
          summary: 'OCR preview sem repositório',
          security: [{ bearerAuth: [] }],
        },
        preHandler: [
          server.authenticate,
    ...
    server.get(
      '/operacional/recebimento-avulsos',
      {
        schema: {
          tags: ['operacional'],
          summary: 'Listar processos avulsos com paginação',
          security: [{ bearerAuth: [] }],
          querystring: {
            type: 'object',
            properties: {
      ...
      async (request, reply) => {
        try {
          ...
          const message =
            error instanceof Error ? error.message : 'Erro ao listar processos avulsos';
    ...
    server.post(
      '/operacional/recebimento-avulsos',
      {
        schema: {
          tags: ['operacional'],
          summary: 'Criar processo avulso (sem repositório)',
          security: [{ bearerAuth: [] }],
        },
        preHandler: [
          server.authenticate,
          authorize('operador', 'administrador'),
          validateBody(criarProcessoAvulsoSchema),
        ],
      },
      async (request, reply) => {
        try {
    ...
    server.post(
      '/operacional/repositorios/:id/recebimento-processos/batch',
      {
        schema: {
          tags: ['operacional'],
          summary: 'Adicionar processos em lote a repositório',
          security: [{ bearerAuth: [] }],
        ...
        preHandler: [
          server.authenticate,
          authorize('operador', 'administrador'),
          validateBody(criarProcessosBatchSchema),
        ],
      },
      async (request, reply) => {
        try {
          ...
          const message =
            error instanceof Error ? error.message : 'Erro ao adicionar processos em lote';
          return reply.status(400).send({ error: message });
        }
      }
    );
    ...
    server.patch(
      '/operacional/recebimento-processos/vincular',
      {
        schema: {
          tags: ['operacional'],
          summary: 'Vincular processos avulsos a repositório',
          security: [{ bearerAuth: [] }],
        },
        preHandler: [
          server.authenticate,
          authorize('operador', 'administrador'),
          validateBody(vincularProcessosSchema),
        ],
      },
      async (request, reply) => {
        try {
          ...
          const encontrados = checkResult.rows as Array<{
            id: string;
    ...
    server.patch(
      '/operacional/recebimento-processos/desvincular',
      {
        schema: {
          tags: ['operacional'],
          summary: 'Desvincular processos de repositório',
          security: [{ bearerAuth: [] }],
        },
        preHandler: [server.authenticate, authorize('operador', 'administrador')],
      },
