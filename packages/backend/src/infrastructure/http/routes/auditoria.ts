...
export function createAuditoriaRoutes(): FastifyPluginAsync {
  return async (server: FastifyInstance): Promise<void> => {
    ...
    server.get('/auditoria', {
      ...
      try {
        const { tabela, operacao, dataInicio, dataFim, pagina = 1, limite = 50 } = request.query as {
          tabela?: string;
          operacao?: string;
          dataInicio?: string;
          dataFim?: string;
        ...
        if (tabela) {
          const tabelas = tabela.split(',').map(t => t.trim()).filter(Boolean);
          if (tabelas.length === 1) {
    ...
    server.get('/auditoria/estatisticas', {
      schema: {
        tags: ['auditoria'],
