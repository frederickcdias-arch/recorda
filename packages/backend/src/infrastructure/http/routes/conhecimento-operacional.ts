...
export function createConhecimentoOperacionalRoutes(): FastifyPluginAsync {
  return async (server: FastifyInstance): Promise<void> => {
    ...
    server.get('/operacional/conhecimento/documentos', {
      schema: { tags: ['conhecimento'], summary: 'Listar documentos da base de conhecimento', security: [{ bearerAuth: [] }], querystring: { type: 'object', properties: { categoria: { type: 'string' }, etapa: { type: 'string' }, busca: { type: 'string' }, pagina: { type: 'number' }, limite: { type: 'number' } } } },
    ...
    server.get('/operacional/conhecimento/documentos/:id', {
      schema: { tags: ['conhecimento'], summary: 'Buscar documento por ID com versões', security: [{ bearerAuth: [] }], params: { type: 'object', properties: { id: { type: 'string' } }, required: ['id'] } },
      preHandler: [server.authenticate, authorize('operador', 'administrador')],
    }, async (request, reply) => {
      try {
        const { id } = request.params as { id: string };
        const user = getCurrentUser(request);
        const whereAccess = user.perfil === 'operador' ? ` AND d.nivel_acesso = 'OPERADOR_ADMIN' AND d.status = 'ATIVO'` : '';
    ...
    server.post('/operacional/conhecimento/documentos', {
      schema: { tags: ['conhecimento'], summary: 'Criar documento na base de conhecimento', security: [{ bearerAuth: [] }] },
      ...
      try {
        ...
        if (!body.codigo || !body.titulo || !body.categoria || !body.conteudo) {
          return reply.status(400).send({ error: 'Campos obrigatorios: codigo, titulo, categoria, conteudo' });
        }
    ...
    server.post('/operacional/conhecimento/documentos/:id/versoes', {
      schema: { tags: ['conhecimento'], summary: 'Criar nova versão de documento', security: [{ bearerAuth: [] }], params: { type: 'object', properties: { id: { type: 'string' } }, required: ['id'] } },
      preHandler: [server.authenticate, authorize('administrador')],
    }, async (request, reply) => {
      try {
        const { id } = request.params as { id: string };
        ...
        const versionResult = await server.database.query<{ id: string }>(
          `INSERT INTO kb_documento_versoes (
           ...
           VALUES ($1, $2, $3, $4, $5)
           RETURNING id`,
          [id, nextVersion, body.conteudo, body.resumoAlteracao?.trim() ?? `Atualizacao v${nextVersion}`, user.id]
        );
    ...
    server.patch('/operacional/conhecimento/documentos/:id', {
      schema: { tags: ['conhecimento'], summary: 'Atualizar metadados de documento', security: [{ bearerAuth: [] }], params: { type: 'object', properties: { id: { type: 'string' } }, required: ['id'] } },
      preHandler: [server.authenticate, authorize('administrador')],
    }, async (request, reply) => {
      try {
        const { id } = request.params as { id: string };
        ...
        const updateResult = await server.database.query(
          `UPDATE kb_documentos
           ...
           WHERE id = $1
           RETURNING *`,
          [id, body.titulo?.trim() ?? null, body.descricao?.trim() ?? null, body.status ?? null, body.nivelAcesso ?? null]
        );
    ...
    server.get('/operacional/conhecimento/glossario', {
      schema: { tags: ['conhecimento'], summary: 'Listar termos do glossário', security: [{ bearerAuth: [] }] },
      preHandler: [server.authenticate, authorize('operador', 'administrador')],
    }, async (request, reply) => {
      try {
        const user = getCurrentUser(request);
        ...
        return reply.status(500).send({ error: error instanceof Error ? error.message : 'Erro ao listar glossário' });
      }
    });

    // POST /operacional/conhecimento/glossario
    server.post('/operacional/conhecimento/glossario', {
      schema: { tags: ['conhecimento'], summary: 'Criar termo no glossário', security: [{ bearerAuth: [] }] },
      preHandler: [server.authenticate, authorize('administrador')],
    }, async (request, reply) => {
      try {
        const user = getCurrentUser(request);
        ...
        return reply.status(400).send({ error: error instanceof Error ? error.message : 'Erro ao criar termo' });
      }
    });

    // PATCH /operacional/conhecimento/glossario/:id
    server.patch<{ Params: { id: string } }>('/operacional/conhecimento/glossario/:id', {
      schema: { tags: ['conhecimento'], summary: 'Atualizar termo do glossário', security: [{ bearerAuth: [] }] },
      preHandler: [server.authenticate, authorize('administrador')],
    }, async (request, reply) => {
      try {
        const { id } = request.params;
        const body = request.body as { termo?: string; definicao?: string; ativo?: boolean; ordem?: number };
        const result = await server.database.query(
          `UPDATE kb_glossario
           ...
           WHERE id = $1
           RETURNING *`,
          [id, body.termo?.trim() ?? null, body.definicao?.trim() ?? null, body.ativo ?? null, body.ordem ?? null]
        );
        if (result.rows.length === 0) return reply.status(404).send({ error: 'Termo não encontrado' });
        return reply.send(result.rows[0]);
      } catch (error) {
        return reply.status(400).send({ error: error instanceof Error ? error.message : 'Erro ao atualizar termo' });
      }
    });

    // DELETE /operacional/conhecimento/glossario/:id
    server.delete<{ Params: { id: string } }>('/operacional/conhecimento/glossario/:id', {
      schema: { tags: ['conhecimento'], summary: 'Excluir termo do glossário', security: [{ bearerAuth: [] }] },
      preHandler: [server.authenticate, authorize('administrador')],
    }, async (request, reply) => {
      try {
        const { id } = request.params;
        const result = await server.database.query('DELETE FROM kb_glossario WHERE id = $1 RETURNING id', [id]);
        if (result.rows.length === 0) return reply.status(404).send({ error: 'Termo não encontrado' });
        return reply.send({ message: 'Termo excluído' });
      } catch (error) {
        return reply.status(500).send({ error: error instanceof Error ? error.message : 'Erro ao excluir termo' });
      }
    });
    ...
    server.get('/operacional/conhecimento/leis-normas', {
      schema: { tags: ['conhecimento'], summary: 'Listar leis e normas', security: [{ bearerAuth: [] }] },
      preHandler: [server.authenticate, authorize('operador', 'administrador')],
    }, async (request, reply) => {
      try {
        const user = getCurrentUser(request);
        ...
        return reply.status(500).send({ error: error instanceof Error ? error.message : 'Erro ao listar leis e normas' });
      }
    });

    // POST /operacional/conhecimento/leis-normas
    server.post('/operacional/conhecimento/leis-normas', {
      schema: { tags: ['conhecimento'], summary: 'Criar lei/norma', security: [{ bearerAuth: [] }] },
      preHandler: [server.authenticate, authorize('administrador')],
    }, async (request, reply) => {
      try {
        const user = getCurrentUser(request);
        const body = request.body as { nome?: string; descricao?: string; referencia?: string; url?: string; ordem?: number };
        ...
        const result = await server.database.query<{ id: string }>(
          `INSERT INTO kb_leis_normas (nome, descricao, referencia, url, ordem, criado_por)
           ...
           RETURNING id`,
          [body.nome.trim(), body.descricao.trim(), body.referencia?.trim() ?? '', body.url?.trim() ?? null, body.ordem ?? 0, user.id]
        );
        return reply.status(201).send({ id: result.rows[0]?.id });
      } catch (error) {
        return reply.status(400).send({ error: error instanceof Error ? error.message : 'Erro ao criar lei/norma' });
      }
    });

    // PATCH /operacional/conhecimento/leis-normas/:id
    server.patch<{ Params: { id: string } }>('/operacional/conhecimento/leis-normas/:id', {
      schema: { tags: ['conhecimento'], summary: 'Atualizar lei/norma', security: [{ bearerAuth: [] }] },
      preHandler: [server.authenticate, authorize('administrador')],
    }, async (request, reply) => {
      try {
        const { id } = request.params;
        const body = request.body as { nome?: string; descricao?: string; referencia?: string; url?: string; ativo?: boolean; ordem?: number };
        const result = await server.database.query(
          `UPDATE kb_leis_normas
           ...
           WHERE id = $1
           RETURNING *`,
          [id, body.nome?.trim() ?? null, body.descricao?.trim() ?? null, body.referencia?.trim() ?? null, body.url?.trim() ?? null, body.ativo ?? null, body.ordem ?? null]
        );
        if (result.rows.length === 0) return reply.status(404).send({ error: 'Lei/norma não encontrada' });
        return reply.send(result.rows[0]);
      } catch (error) {
        return reply.status(400).send({ error: error instanceof Error ? error.message : 'Erro ao atualizar lei/norma' });
      }
    });

    // DELETE /operacional/conhecimento/leis-normas/:id
    server.delete<{ Params: { id: string } }>('/operacional/conhecimento/leis-normas/:id', {
      schema: { tags: ['conhecimento'], summary: 'Excluir lei/norma', security: [{ bearerAuth: [] }] },
      preHandler: [server.authenticate, authorize('administrador')],
    }, async (request, reply) => {
      try {
        const { id } = request.params;
        const result = await server.database.query('DELETE FROM kb_leis_normas WHERE id = $1 RETURNING id', [id]);
        if (result.rows.length === 0) return reply.status(404).send({ error: 'Lei/norma não encontrada' });
        return reply.send({ message: 'Lei/norma excluída' });
      } catch (error) {
        return reply.status(500).send({ error: error instanceof Error ? error.message : 'Erro ao excluir lei/norma' });
      }
    });
  };
}
