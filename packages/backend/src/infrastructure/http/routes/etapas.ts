...
export function createEtapasRoutes(): FastifyPluginAsync {
  return async (server: FastifyInstance): Promise<void> => {
      ...
      '/etapas',
      {
        schema: {
          ...
          querystring: {
            type: 'object',
            properties: {
              ativa: { type: 'string', enum: ['true', 'false'] },
    ...
    server.post<{ Body: CriarEtapaBody }>(
      '/etapas',
      {
        schema: {
          tags: ['etapas'],
    ...
    server.put<{ Params: { id: string }; Body: CriarEtapaBody }>(
      '/etapas/:id',
      {
        schema: {
          tags: ['etapas'],
      ...
      async (request, reply) => {
        try {
          ...
          if (novaOrdem !== ordemAtual) {
            ...
            try {
              ...
              if (conflito.rows.length > 0) {
                ...
                await server.database.query(`UPDATE etapas SET ordem = $1 WHERE id = $2`, [
                  ordemAtual,
                  conflito.rows[0]?.id,
                ]);
                await server.database.query(
                  `UPDATE etapas SET nome = $1, descricao = $2, unidade = $3, ordem = $4 WHERE id = $5`,
                  [nome, descricao || null, unidade || 'PROCESSO', novaOrdem, id]
                );
              } else {
                await server.database.query(
                  `UPDATE etapas SET nome = $1, descricao = $2, unidade = $3, ordem = $4 WHERE id = $5`,
          ...
          const updateResult = await server.database.query(`SELECT * FROM etapas WHERE id = $1`, [
            id,
          ]);
          ...
          if (message.includes('invalid input value for enum')) {
            return reply.status(400).send({
              error: 'Unidade de medida inválida. Use: PROCESSO, VOLUME, PAGINA ou DOCUMENTO',
            });
          }
          return reply.status(500).send({ error: message });
        }
    ...
    server.patch<{ Params: { id: string }; Body: { direcao: 'up' | 'down' } }>(
      '/etapas/:id/reordenar',
      {
        schema: {
          tags: ['etapas'],
          ...
          body: {
            type: 'object',
            required: ['direcao'],
            properties: { direcao: { type: 'string', enum: ['up', 'down'] } },
          },
          response: {
            400: { type: 'object', properties: { error: { type: 'string' } } },
            404: { type: 'object', properties: { error: { type: 'string' } } },
    ...
    server.get<{ Params: { id: string } }>(
      ...
      async (request, reply) => {
        try {
          ...
          const result = await server.database.query(`SELECT * FROM etapas WHERE id = $1`, [id]);
