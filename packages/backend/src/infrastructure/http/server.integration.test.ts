...
function createMockDatabase(): DatabaseConnection & {
  queryMock: Mock<(sql: string, params?: unknown[]) => Promise<QueryResult<QueryResultRow>>>;
  usuarios: Map<
    string,
    {
      id: string;
  ...
  const queryMock: Mock<(sql: string, params?: unknown[]) => Promise<QueryResult<QueryResultRow>>> =
    vi.fn(async (text: string, params?: unknown[]): Promise<QueryResult<QueryResultRow>> => {
      ...
      if (
        text.includes('FROM repositorios') &&
        text.includes('WHERE id_repositorio_recorda = $1')
      ) {
        const id = String(params?.[0]);
        for (const repo of repositorios.values()) {
      ...
      if (text.includes('UPDATE repositorios SET etapa_atual')) {
        return makeResult(
          [{ id_repositorio_recorda: String(params?.[2] ?? params?.[0]) }],
          'UPDATE'
        );
      }
      ...
      if (
        text.includes('UPDATE fontes_importacao SET ultima_importacao_em = NOW() WHERE id = $1')
      ) {
        return makeResult([], 'UPDATE');
      }
      ...
      if (
        text.includes('DELETE FROM recebimento_documentos') &&
        text.includes('WHERE origem = $1')
      ) {
      ...
      if (
        ...
        if (
          !destinatario ||
          !comunicado ||
          !['PUBLICADO', 'ENCERRADO'].includes(comunicado.status)
        ) {
          return makeResult([], 'UPDATE');
        }
      ...
      if (text.includes('UPDATE lotes_cq SET status')) {
        return makeResult(
          [{ id: String(params?.[0] ?? params?.[1]), status: 'FECHADO' }],
          'UPDATE'
        );
      }
